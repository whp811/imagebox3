...
cp -f "$ROOT/packaging/usb-production/Launch-WSI-Hive-Windows.hta" "$OUT/" 2>/dev/null || true
cp -R "$ROOT/packaging/usb-production/Launch-WSI-Hive-Mac.app" "$OUT/" 2>/dev/null || true
