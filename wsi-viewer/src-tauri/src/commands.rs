...
#[tauri::command]
pub fn slides_set_session_root(
    path: Option<String>,
...
pub fn wsi_path_to_url(absolute_path: String) -> Result<String, String> {
    ...
    let cache = paths::cache_root_dir();
    let materialized =
        crate::zip_ops::materialize_zip_entry_source_for_viewing(&absolute_path, &cache)
            .map_err(|e| e.to_string())?;
...
pub fn wsi_embedded_label_thumbnail(absolute_path: String) -> Option<String> {
    let _ = paths::ensure_cache_dir();
    embedded_label_thumbnail::read_embedded_label_thumbnail_data_url(
        &absolute_path,
