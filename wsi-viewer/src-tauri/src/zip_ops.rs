...
pub fn list_zip_entries(zip_path: &Path) -> Result<Vec<ZipEntryInfo>, Box<dyn std::error::Error + Send + Sync>> {
...
pub fn get_zip_entry_info(zip_path: &Path, entry_name: &str) -> Result<Option<ZipEntryInfo>, Box<dyn std::error::Error + Send + Sync>> {
...
fn zip_cache_path(
  ...
  cache_root.join("zip-cache").join(format!("{digest24}{ext}"))
}
...
pub fn materialize_zip_entry_source_for_viewing(
  ...
  if info.compression_method != 8 {
    return Err(format!("Unsupported ZIP compression method: {}", info.compression_method).into());
  }
...
pub fn read_stored_zip_entry_range(
  ...
  let data_start = {
    ...
    if z.compression() != CompressionMethod::Stored {
      return Err("ZIP slide entries must be stored without compression for WSI range reads".into());
    }
