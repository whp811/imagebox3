...
fn read_at(kind: &SourceKind, pos: u64, len: usize) -> std::io::Result<Vec<u8>> {
    match kind {
        ...
        SourceKind::ZipStored {
            ...
            read_stored_zip_entry_range(zip_path, entry_name, pos, end)
...
fn read_tag_data(
    ...
    let byte_len = (type_size(ty) as u64)
        .saturating_mul(count)
        .min(MAX_TAG_BYTES as u64) as usize;
    ...
    for index in 0..MAX_IFDS {
        ...
        for entry_index in 0..entry_count as usize {
            ...
            if tag == TAG_IMAGE_DESCRIPTION && ty == TYPE_ASCII {
                if let Some(data) =
                    read_tag_data(kind, tiff_base, entry, ty, count, endian, big_tiff)
                {
            ...
            let Some(data) = read_tag_data(kind, tiff_base, entry, ty, count, endian, big_tiff)
            else {
                continue;
            };
            ...
            match tag {
                ...
                TAG_BITS_PER_SAMPLE => {
                    ifd.bits_per_sample = values.iter().map(|&v| v as u32).collect()
                }
...
fn is_supported_candidate(ifd: &TiffIfd) -> bool {
    ...
    let Some(c) = ifd.compression else {
        return false;
    };
